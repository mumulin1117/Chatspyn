...
final class GoatspynAgreementPyn: UIViewController {
    ...
    private func deployBaseArenaPyn() {
        obscureOverlayPyn.frame = view.bounds
        obscureOverlayPyn.backgroundColor = UIColor.black.withAlphaComponent(0.5)
        view.addSubview(obscureOverlayPyn)
      
        consensusSheetPyn.frame = CGRect(x: 0, y: verticalPyn - 500, width: horizonPyn, height: 500)
        ...
        Pyntitle.frame = CGRect.init(x: 0, y: 20, width: horizonPyn, height: 30)
        ...
        License Grant
        Chatspyn grants you a revocable, non-exclusive, non-transferable, limited license to download and use the app solely for your personal, non-commercial sports and social purposes.

        User Behavior Restrictions
        To ensure the "energy" of the community remains positive, you agree NOT to:
        ...
        let btnWidthPyn = (horizonPyn - 60) / 2
     
        legalTitlePyn.frame = CGRect.init(x: 12, y: Pyntitle.frame.maxY + 20, width: horizonPyn - 24, height: 286)
